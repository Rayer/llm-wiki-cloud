...
func NewProductionExecutor(conceptCache *cache.Cache, provider ChatProvider, legacy query.Executor, synthesizer *query.Service, options Options) (query.Executor, error) {
	...
	queryRetrievalPipeline, err := NewQueryRetrievalPipelineWithOptionsAndProfile(expander, NewLexicalMatcher(nil), NewResultSelector(), options.SeedFor, options, DefaultRetrievalProfile())
	if err != nil {
		return nil, err
	}
