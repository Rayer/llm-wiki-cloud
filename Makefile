...
deploy-dev:
	gcloud run deploy llm-wiki-bff-dev --project llm-wiki-cloud --image $(IMAGE) --region asia-east1 --platform managed --allow-unauthenticated --service-account lwc-bff-dev@llm-wiki-cloud.iam.gserviceaccount.com --update-secrets "JWT_SECRET=jwt-secret-dev:latest,DEEPSEEK_API_KEY=deepseek-apikey:*** --update-env-vars "^@^GCP_PROJECT=llm-wiki-cloud@BUCKET=llm-wiki-data-dev@FIRESTORE_DATABASE_ID=llm-wiki-cloud-dev@PIPELINE_JOB_URL=https://run.googleapis.com/v2/projects/llm-wiki-cloud/locations/asia-east1/jobs/olw-pipeline-dev:run@ALLOWED_ORIGINS=https://wiki.dev.rayer.idv.tw,https://llm-wiki-frontend-dev.vercel.app,http://localhost:3000,http://127.0.0.1:3000@AUTH_SERVICE_URL=https://auth.dev.rayer.idv.tw@DEV_JWT=false" --port 8080
