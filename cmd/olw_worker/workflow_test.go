...
func TestWorkerPromotionWorkflowsContract(t *testing.T) {
	...
	for _, want := range []string{"scripts/render_worker_deployment_evidence.py render-evidence", "--bucket \"$BUCKET\"", "--rollback-contract \"$ROLLBACK_CONTRACT\"", "--metadata \"$METADATA\"", "--output \"$EVIDENCE\"", "--arg rollback_artifact_name \"$EVIDENCE_ARTIFACT_NAME\""} {
