...
Each custom role must already match its exact reviewed permission set if present. Resource creation is create-only; a mismatched existing resource stops the run. IAM changes capture the complete policy and etag before each set, retry only etag conflicts after a fresh read, preserve concurrent additions, and verify exact bindings by read-back. The evidence artifact records created resources and exact inverse bindings; resource deletion requires a separate consumer check.
