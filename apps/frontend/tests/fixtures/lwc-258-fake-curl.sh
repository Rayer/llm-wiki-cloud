...
if [[ "$url" == https://api.github.com/repos/Rayer/llm-wiki-cloud/actions/workflows/ci.yml/runs* ]]; then
  ...
  elif [[ "$FIXTURE_SCENARIO" == post-create-ready-routing ]]; then jq '.deployments.dpl_new123 | del(.alias) + {aliasAssigned:true,userAliases:["dpl-new-git-main.vercel.app"],automaticAliases:["dpl-new-git-main-rayer-tung-s-projects.vercel.app"],aliasError:null,aliasWarning:null,aliasFinal:"dpl-new-final.vercel.app"}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-user-alias-overlap ]]; then jq '.deployments.dpl_new123 + {aliasAssigned:true,alias:["dpl-new-git-main.vercel.app"],userAliases:["dpl-new-git-main.vercel.app"],aliasError:null,aliasWarning:null}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-final-overlap ]]; then jq '.deployments.dpl_new123 + {alias:["dpl-new-git-main.vercel.app"],aliasFinal:"dpl-new-git-main.vercel.app",aliasError:null,aliasWarning:null}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-assigned ]]; then jq '.deployments.dpl_new123 + {aliasAssigned:true}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-canonical-alias ]]; then jq '.deployments.dpl_new123 + {alias:["wiki.rayer.idv.tw"]}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-canonical-user-alias ]]; then jq '.deployments.dpl_new123 + {userAliases:["llm-wiki-frontend.vercel.app"]}' "$state"
  ...
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-host-malformed ]]; then jq '.deployments.dpl_new123 + {alias:["https://dpl-new.vercel.app/*"]}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-duplicate ]]; then jq '.deployments.dpl_new123 + {alias:["dpl-new.vercel.app","DPL-NEW.VERCEL.APP"]}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-user-alias-malformed ]]; then jq '.deployments.dpl_new123 + {userAliases:"dpl-new.vercel.app"}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-automatic-alias-malformed ]]; then jq '.deployments.dpl_new123 + {automaticAliases:["dpl-new.vercel.app", 7]}' "$state"
  ...
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-assigned-malformed ]]; then jq '.deployments.dpl_new123 + {aliasAssigned:"true"}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-error ]]; then jq '.deployments.dpl_new123 + {aliasError:{code:"ALIAS_FAILED",message:"alias failed"}}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-warning ]]; then jq '.deployments.dpl_new123 + {aliasWarning:{code:"ALIAS_WARNING",message:"alias warning"}}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-canonical-alias-final ]]; then jq '.deployments.dpl_new123 + {aliasFinal:"wiki.rayer.idv.tw"}' "$state"
  elif [[ "$FIXTURE_SCENARIO" == post-create-alias-final-malformed ]]; then jq '.deployments.dpl_new123 + {aliasFinal:"https://dpl-new.vercel.app/path"}' "$state"
